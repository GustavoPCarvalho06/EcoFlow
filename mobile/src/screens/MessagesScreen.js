...
export default function MessagesScreen({ navigation }) {
  ...
  // Listener de Socket para atualizar lista em tempo real
  useEffect(() => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // 1. Busca contagens
      const resCounts = await fetch(`${API_URL}/msg/unread-counts`, { headers });
      if (resCounts.ok) {
        const dataCounts = await resCounts.json();
        setUnreadPerUser(dataCounts);
      }
      
      // 2. Busca usuários (apenas se a lista estiver vazia para economizar dados)
      if (users.length === 0) {
        const resUsers = await fetch(`${API_URL}/user/paginated?statusConta=ativo&limit=1000`, { headers });
        if (resUsers.ok) {
      ...
      >
        ...
        <View style={styles.infoContainer}>
            ...
            <Text style={styles.preview} numberOfLines={1}>
                {count > 0 ? "Novas mensagens" : "Toque para conversar"}
            </Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  searchContainer: { flexDirection: 'row', backgroundColor: '#f1f1f1', margin: 10, borderRadius: 10, alignItems: 'center', paddingHorizontal: 10 },
  ...
  avatarContainer: { width: 50, height: 50, borderRadius: 25, backgroundColor: '#e1e1e1', justifyContent: 'center', alignItems: 'center', marginRight: 15 },
  ...
  badge: { backgroundColor: '#28a745', minWidth: 22, height: 22, borderRadius: 11, justifyContent: 'center', alignItems: 'center', paddingHorizontal: 6, marginLeft: 5 },
